...
def profile_model_comparison():
    """Complete model comparison with profiling - FIXED FOR SPLIT FILES"""
    profiler = ModelProfiler()
    
    # Load and prepare data (common) - FIXED: Use main processing logic
    train_df = pd.read_csv(TRAIN_FILE)
    train_df['Date'] = pd.to_datetime(train_df['Date'])
    train_df['Store'] = STORE_ID  # Set store ID as done in main logic
    
    # FIXED: Filter by Store after setting it
    train_series = train_df[train_df['Store'] == STORE_ID]['value'].values
    n_weeks = len(train_series)
    ...
    model = ARIMA(train_series, order=(7,0,7)).fit()
    compile_time, memory_gb = profiler.end_timer()
    arima_flops = profiler.estimate_flops('ARIMA', n_weeks)
    comparison_results.append(['ARIMA(7,0,7)', compile_time, memory_gb, arima_flops])
    
    # === STL-ARIMA ===
    print("Profiling STL-ARIMA...")
    profiler.start_timer()
    train_pd = pd.Series(train_series, index=pd.date_range(start='2010-02-05', periods=n_weeks, freq='W-FRI'))
    stl = STL(train_pd, period=52, seasonal=13).fit()
    resid_model = ARIMA(stl.resid, order=(7,0,7)).fit()
    compile_time, memory_gb = profiler.end_timer()
    stl_flops = profiler.estimate_flops('STL-ARIMA', n_weeks)
    comparison_results.append(['STL-ARIMA', compile_time, memory_gb, stl_flops])
    
    # === Prophet ===
    print("Profiling Prophet...")
    profiler.start_timer()
    df_prophet = pd.DataFrame({'ds': pd.date_range(start='2010-02-05', periods=n_weeks, freq='W-FRI'), 'y': train_series})
    model = Prophet(yearly_seasonality=True, weekly_seasonality=False)
    model.fit(df_prophet)
    compile_time, memory_gb = profiler.end_timer()
    prophet_flops = profiler.estimate_flops('Prophet', n_weeks)
...
if __name__ == "__main__":
    ...
    # 1. LOAD TRAIN & TEST DATA (common to all)
    print("Loading files...")
    train_df = pd.read_csv(TRAIN_FILE)
    ...
    metrics = get_metrics(train_series[-FORECAST_HORIZON:], fc_arima, train_series)
    row = ['ARIMA(7,0,7)', metrics['MAE'], metrics['sMAPE'], metrics['WMAPE'], metrics['MASE'], metrics['WQL']]
    results.append(row)
    forecasts['ARIMA'] = fc_arima
    
    # STL + ARIMA
    if len(train_series) > 2 * SEASONALITY:
        # Use pandas Series with index → STL can infer frequency
        train_pd = pd.Series(train_series, index=pd.to_datetime(train_dates))
        train_pd = train_pd.asfreq('W-FRI', method='ffill')  # ensure weekly freq
        
        stl = STL(train_pd, period=SEASONALITY, seasonal=13).fit()  # period=52, seasonal=13 is common
        ...
        fc_trend = np.full(FORECAST_HORIZON, last_trend)  # or linear extrapolation if desired
        
        # Seasonal repeats from end of series
    ...
    metrics = get_metrics(train_series[-FORECAST_HORIZON:], stl_forecast, train_series)
    row = ['STL_ARIMA', metrics['MAE'], metrics['sMAPE'], metrics['WMAPE'], metrics['MASE'], metrics['WQL']]
    results.append(row)
    ...
    metrics = get_metrics(train_series[-FORECAST_HORIZON:], prophet_forecast, train_series)
    row = ['Prophet', metrics['MAE'], metrics['sMAPE'], metrics['WMAPE'], metrics['MASE'], metrics['WQL']]
    results.append(row)
    ...
    for _ in range(200):
        ...
        # LSTM forecasting – use numpy array internally for stability
        curr = scaled_train[-seq_len:].copy()  # shape (seq_len, 1)
        
        lstm_fc_scaled = []
        for _ in range(FORECAST_HORIZON):
            inp = torch.from_numpy(curr[-seq_len:]).float().unsqueeze(0)  # (1, seq_len, 1)
            ...
            # Append new prediction (as 2D for consistency)
            curr = np.append(curr, [[out]], axis=0)
    
    lstm_forecast = scaler.inverse_transform(np.array(lstm_fc_scaled).reshape(-1, 1)).flatten()
    metrics = get_metrics(train_series[-FORECAST_HORIZON:], lstm_forecast, train_series)
    row = ['LSTM', metrics['MAE'], metrics['sMAPE'], metrics['WMAPE'], metrics['MASE'], metrics['WQL']]
    results.append(row)
    forecasts['LSTM'] = lstm_forecast
    
    # TRAIN METRICS
    df_train = pd.DataFrame(results, columns=['Model', 'MAE', 'sMAPE', 'WMAPE', 'MASE', 'WQL'])
    print("\n" + "="*80)
    print("TRAIN METRICS (Last 12 weeks of training)")
    print("="*80)
    print(df_train.round(4).to_string(index=False))
    
    df_train.to_csv('walmart_metrics_train_last12.csv', index=False)
    
    # TEST METRICS
    print("\n" + "="*80)
    print("TEST METRICS vs ACTUAL")
    print("="*80)
    
    test_results = []
    for name, fc in forecasts.items():
        mt = get_metrics(test_series, fc, train_series)   # ← Pass full train for MASE
        test_results.append(['Test – ' + name, mt['MAE'], mt['sMAPE'], mt['WMAPE'], mt['MASE'], mt['WQL']])
    
    df_test = pd.DataFrame(test_results, columns=['Model', 'MAE', 'sMAPE', 'WMAPE', 'MASE', 'WQL'])
    print(df_test.round(4).to_string(index=False))
    
    df_test.to_csv('walmart_metrics_test.csv', index=False)
    
    # Save forecast vs actual
    ...
    # Save STL_ARIMA forecast for nudger (as 'stl_arima_actual_vs_predicted.csv')
    stl_arima_df = pd.DataFrame({
        'Week': [f"Week {i+1}" for i in range(FORECAST_HORIZON)],
        'Actual': test_series,
        'STL_ARIMA': forecasts['STL_ARIMA']
    })
    stl_arima_df.to_csv('stl_arima_actual_vs_predicted.csv', index=False)
    print("Saved STL_ARIMA forecast → stl_arima_actual_vs_predicted.csv")
    
    # Plot baselines
    plt.figure(figsize=(14, 7))
    plt.plot(test_dates, test_series, label='Actual Test', color='darkgreen', linewidth=2.8, marker='o', markersize=6)
    
    for name, fc in forecasts.items():
        plt.plot(test_dates, fc, label=f'{name} Forecast', marker='^', linewidth=1.9, alpha=0.9)
    
    plt.title(f'Walmart Store {STORE_ID} – Test Period vs Model Forecasts')
    plt.xlabel('Date')
    ...
    plt.tight_layout()
    plt.savefig('walmart_forecast_vs_test_plot.png', dpi=200)
    plt.show()
    plt.close()
    
    # ── Nudger ──
    print("=== SMI Nudger on Classical Models Baseline ===\n")
    
    # Load SMI (full, but nudger uses tail(8))
    smi_df = pd.read_csv(smi_output_file)
    
    # Load Classical Models baseline forecast (STL+ARIMA)
    baseline_df = pd.read_csv('stl_arima_actual_vs_predicted.csv')
    # Rename prediction column to 'Demand' for compatibility with nudger
    baseline_df = baseline_df.rename(columns={'STL_ARIMA': 'Demand'})
    
    print(f"Loaded Classical Models forecast: {len(baseline_df)} weeks, "
          f"avg {baseline_df['Demand'].mean():,.0f}/week")
    
    # Apply nudger
    nudged_df = apply_nudger_to_12weeks(baseline_df, smi_df)
    
    # Load actual test data (already loaded, but reuse)
    actual_weekly_values = test_series[:12]  # assuming FORECAST_HORIZON=12
    plot_dates = test_dates[:12]
    
    print(f"Test actuals (first 12 weeks): {len(actual_weekly_values)} weeks, "
          f"avg {actual_weekly_values.mean():,.2f}")
    
    # Extract predictions
    baseline_weekly = baseline_df['Demand'].head(12).values
    nudged_weekly = nudged_df['nudged_demand'].head(12).values
    
    # Metrics
    history_weekly = smi_df['Weekly_Sales'].values  # Full history from SMI file
    ...
    nudged_df.to_csv('Classical_Models_nudged_12weeks.csv', index=False)
    
    # Metrics table
    metrics_df = pd.DataFrame([
        {"Model": "Classical_Models_Baseline", **{f"Test_{k}": v for k, v in metrics_baseline.items()}},
        {"Model": "SMI_Nudged", **{f"Test_{k}": v for k, v in metrics_nudged.items()}}
    ])
    metrics_df.to_csv('nudged_vs_classical_metrics_weekly.csv', index=False)
    
    # Plot
    plt.figure(figsize=(12, 6))
    plt.plot(plot_dates, actual_weekly_values, label='Actual', color='black', marker='o', linewidth=2)
    plt.plot(plot_dates, baseline_weekly, label='Classical Models Baseline', color='gray', marker='s', linewidth=2)
    plt.plot(plot_dates, nudged_weekly, label='SMI Nudged', color='blue', marker='^', linewidth=2)
    plt.title("12-Week Actual vs Classical Models Baseline vs SMI-Nudged")
    plt.xlabel("Week")
    ...
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig("nudged_vs_actual_classical_models.png", dpi=150)
    plt.show()
    plt.close()
    
    print("\n=== 12-WEEK TEST METRICS (Nudged) ===")
